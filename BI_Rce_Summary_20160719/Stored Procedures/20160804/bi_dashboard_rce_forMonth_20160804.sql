...
BEGIN
	...
	from	(
				select	total_rce_e = sum(isnull(a.total_est_rce, 0)),
						...
						sign_date = convert(varchar(2),datepart(ww, a1.sign_date))
				...
				group by convert(varchar(2),datepart(ww, a1.sign_date))
			) a 
			left outer join
			(
		...
		from	(
					...
					where	sign_date between '2016-01-01' and '2016-12-31'
				) b
				inner join
				(
					select	*,
							...
							total_processed_a = total_rce_a - (total_rce_inactive_a + total_rce_deleted_a)
					from	#bi_dashboard_rce_forMonth
					where	sign_date between '2015-01-01' and '2015-12-31'
				) c on b.sign_date = c.sign_date
		order by sign_date
